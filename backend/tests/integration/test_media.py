...
# -- Supplementary coverage (added alongside the implementation) --

@pytest.mark.django_db
class TestPolyglotAndAttachment:
    def test_trailing_payload_is_stripped(self):
        """A payload appended after the image data must not survive re-encoding
        (docs/MEDIA.md: defuse polyglot files)."""
        user = _make_user("poly@example.com", RoleName.PARTICIPANT)
        client = _auth_client(user)
        smuggled = b"<?php system($_GET[0]); ?>"
        f = io.BytesIO(_make_jpeg_bytes() + smuggled)
        f.name = "polyglot.jpg"
        resp = client.post(UPLOAD_URL, {"file": f}, format="multipart")
        assert resp.status_code == status.HTTP_201_CREATED
        asset = MediaAsset.objects.get(id=resp.data["id"])
        with asset.file.open("rb") as stored:
            assert smuggled not in stored.read()

    def test_asset_can_be_attached_to_an_instance_and_filtered(self):
        from apps.instances.models import WorkflowInstance
        from apps.workflows.models import State, WorkflowDefinition

        user = _make_user("att@example.com", RoleName.PARTICIPANT)
        wf = WorkflowDefinition.objects.create(name="Attach WF")
        State.objects.create(workflow_definition=wf, name="Open", is_initial=True, position_order=1)
        inst = WorkflowInstance.objects.create(workflow_definition=wf)

        client = _auth_client(user)
        f = io.BytesIO(_make_jpeg_bytes())
        f.name = "evidence.jpg"
        resp = client.post(
            UPLOAD_URL, {"file": f, "workflow_instance": str(inst.id)}, format="multipart"
        )
        assert resp.status_code == status.HTTP_201_CREATED, resp.data
        assert str(resp.data["workflow_instance"]) == str(inst.id)

        listed = client.get(f"{UPLOAD_URL}?workflow_instance={inst.id}")
        results = listed.data.get("results", listed.data)
        assert [r["id"] for r in results] == [resp.data["id"]]
